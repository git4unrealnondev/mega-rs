...
#[async_trait]
impl HttpClient for ClientWithMiddleware {
            ...
            let request = async {
                self.post(url.clone())
                    ...
                    .map_err(|e: reqwest_middleware::Error| {
                        // Handle different types of MiddlewareError and convert them
                        // For example, if it's a reqwest::Error wrapped by middleware:
                        if let reqwest_middleware::Error::Reqwest(err) = e {
                            err
                        } else {
                            // Or create a new reqwest::Error from the middleware error
                            panic!();
                        }
                    })?
                    .error_for_status()?
